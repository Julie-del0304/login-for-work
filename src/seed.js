...
function seedUsers() {
  const insertOrIgnoreUser = db.prepare(`
    INSERT OR IGNORE INTO users (employee_id, name, role, mobile, password_hash, biometric_passcode)
    VALUES (?, ?, ?, ?, ?, ?)
  `);
  ...
  for (const user of users) {
    ...
    const result = insertOrIgnoreUser.run(
      user.employeeId,
      user.name,
      user.role,
      user.mobile,
  ...
  return {
    totalTemplateUsers: users.length,
    insertedCount,
...
if (require.main === module) {
  const summary = seedUsers();
  console.log(
    `Seed completed. Inserted: ${summary.insertedCount}, Skipped(existing): ${summary.skippedCount}`
  );
}
